...
const Dashboard: React.FC = () => {
  ...
  const Tile = ({ icon: Icon, label, value, color }: any) => (
    <div className="bg-white border border-[#7FA9B1] p-3 flex items-center gap-3 min-w-[180px]">
  ...
  return (
    <div className="flex flex-col h-full bg-[#F7FAFA] text-black font-sans text-[11px] select-none overflow-auto">
      ...
      <div className="p-4 flex gap-3 flex-wrap">
        <Tile icon={Users} label="Clientes" value={stats?.total_clients} color="#5C8891" />
        <Tile icon={ShieldCheck} label="Clientes ativos" value={stats?.active_clients} color="#5C8891" />
        <Tile icon={FlaskConical} label="Em trial" value={stats?.trial_clients} color="#7FA9B1" />
        <Tile icon={KeyRound} label="Licenças emitidas" value={stats?.total_licenses} color="#5C8891" />
      </div>

      <div className="px-4 pb-4">
        <div className="bg-white border border-[#7FA9B1]">
          <div className="px-2 py-1.5 font-bold text-gray-700 bg-[#F7FAFA] border-b border-[#7FA9B1]">Clientes recentes</div>
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-[#F7FAFA] border-b border-[#7FA9B1] text-gray-700">
                <th className="py-1 px-2 border-r border-[#F7FAFA] font-normal w-24">Código</th>
                <th className="py-1 px-2 border-r border-[#F7FAFA] font-normal">Nome</th>
                <th className="py-1 px-2 border-r border-[#F7FAFA] font-normal w-32 text-center">Ativo</th>
            ...
            <tbody>
              {recentes.map((c) => {
                ...
                return (
                  <tr key={c.id} className="border-b border-[#F7FAFA] hover:bg-[#F7FAFA]">
                    <td className="py-1 px-2 border-r border-[#F7FAFA]">{c.code}</td>
                    <td className="py-1 px-2 border-r border-[#F7FAFA]">{c.commercial_name}</td>
                    <td className="py-1 px-2 border-r border-[#F7FAFA] text-center">
                      {c.status === 'ACTIVE' ? <span className="text-[#5C8891] font-bold">✓</span> : <span className="text-gray-400">-</span>}
                    </td>
