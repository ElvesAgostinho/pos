        ...
        <table className="w-full text-left border-collapse cursor-default">
          <thead>
            <tr className="bg-gradient-to-b from-[#FFFFFF] to-[#EEF4F5] border-b border-[#7FA9B1] text-gray-700">
              <th className="py-1 px-2 border-r border-[#EEF4F5] font-normal w-36">Quando</th>
              <th className="py-1 px-2 border-r border-[#EEF4F5] font-normal w-64">Ação</th>
              <th className="py-1 px-2 border-r border-[#EEF4F5] font-normal w-40">Por</th>
          ...
          <tbody>
            {logs.map((l) => (
              <tr key={l.id} onClick={() => setSelected(l)}
                className={`border-b border-[#F7FAFA] hover:bg-[#F7FAFA] cursor-pointer ${ACOES_FALHA.has(l.action) ? 'bg-[#FDECEA]' : ''}`}>
                <td className="py-1 px-2 border-r border-[#F7FAFA] whitespace-nowrap" title={l.timestamp}>{tempoRelativo(l.timestamp)}</td>
                <td className="py-1 px-2 border-r border-[#F7FAFA]">
                  <span className={ACOES_FALHA.has(l.action) ? 'text-[#8C2B1F] font-bold flex items-center gap-1' : ''}>
                    {ACOES_FALHA.has(l.action) && <ShieldAlert size={11} />}
                    {ACTION_LABEL[l.action] || l.action}
                  </span>
                </td>
                <td className="py-1 px-2 border-r border-[#F7FAFA]">{l.user_identity}</td>
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center" onClick={() => setSelected(null)}>
          <div className="bg-[#F7FAFA] border border-[#7FA9B1] w-[520px] shadow-md flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4 bg-white space-y-2 text-[11px]">
              ...
              <pre className="bg-[#F7FAFA] border border-[#EEF4F5] p-2 mt-2 whitespace-pre-wrap break-all text-[10px]">
