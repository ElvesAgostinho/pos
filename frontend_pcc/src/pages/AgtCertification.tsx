  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      <div className="bg-white border border-gray-300 rounded p-4 space-y-3 shadow-sm">
        ...
        <button onClick={generate} disabled={busy} className="bg-[#062A31] hover:bg-[#041F24] text-white text-sm px-4 py-2 rounded disabled:opacity-50">
      ...
      {result && (
        <div className="bg-white border border-[#CFE3E6] rounded p-4 mt-4 space-y-3 shadow-sm">
          <div className="text-[#041F24] font-bold text-sm">✓ Credenciais geradas — entregue ao cliente</div>
          ...
          {[['Chave privada (PEM)', result.private_key], ['Chave pública (PEM)', result.public_key]].map(([label, val]: any) => (
            <div key={label}>
              <div className="flex items-center justify-between mb-1"><span className="text-xs font-bold text-gray-600">{label}</span>
                <button onClick={() => copy(val)} className="text-xs text-[#062A31] hover:underline">copiar</button></div>
      ...
      <div className="bg-white border border-gray-300 rounded p-4 mt-6 space-y-3 shadow-sm">
        ...
        {!licenseId && <div className="text-sm text-[#062A31] bg-[#F7FAFA] border border-[#EEF4F5] rounded px-3 py-2">Escolha a licença do cliente no topo da página para configurar a ligação.</div>}

        {licenseId && (
          <>
            {estado && (
              <div className={`text-xs rounded px-3 py-2 border ${estado.configured ? 'bg-[#F7FAFA] border-[#EEF4F5] text-[#041F24]' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
            ...
            <div className="flex items-center gap-3 pt-1">
              <button onClick={gravarLigacao} disabled={busyCon} className="bg-[#062A31] hover:bg-[#041F24] text-white text-sm px-4 py-2 rounded disabled:opacity-50">
                {busyCon ? 'A gravar…' : 'Gravar ligação e-fatura'}
              </button>
              {msgCon && <span className="text-sm text-[#062A31]">{msgCon}</span>}
            </div>
