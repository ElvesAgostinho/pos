...
export default function ClassicButton({ icon: Icon, iconColor = 'text-gray-700', label, className = '', ...props }: ClassicButtonProps) {
  return (
    <button 
      className={`flex items-center space-x-1.5 px-3 py-1.5 bg-[#F7FAFA] border border-[#7FA9B1] shadow-[inset_1px_1px_0px_#FFFFFF] text-[11px] font-sans focus:outline-none focus:ring-1 focus:ring-black ${props.disabled ? 'opacity-50 cursor-not-allowed text-gray-500' : 'hover:bg-[#F7FAFA] active:bg-[#EEF4F5] active:shadow-[inset_1px_1px_2px_rgba(0,0,0,0.2)] text-gray-800'} ${className}`}
      {...props}
    >
      {Icon && (
