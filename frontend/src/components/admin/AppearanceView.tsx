...
// azul (#5C8891) diferente do resto: bastava visitar este ecrã, ou carregar
...
function ImagePicker({ label, storageKey, hint }: { label: string; storageKey: string; hint: string }) {
  ...
  return (
    <div className="bg-[#F7FAFA] border border-[#CFE3E6] p-3">
      <div className="flex items-center gap-2 mb-2 text-[#5C8891] font-bold text-[12px]"><Image size={14} />{label}</div>
      <div className="flex gap-3">
        <div className="w-40 h-24 bg-white border border-[#7FA9B1] flex items-center justify-center overflow-hidden">
        ...
        <div className="flex-1 text-[11px] text-gray-600">
          ...
          <label className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#F7FAFA] border border-[#7FA9B1] shadow-[inset_1px_1px_0_#FFFFFF] text-[11px] cursor-pointer hover:bg-[#F7FAFA]">
            <Upload size={13} /> Escolher imagem…
            <input type="file" accept="image/*" className="hidden" onChange={(e) => onFile(e.target.files?.[0])} />
          </label>
          {val && <button onClick={clear} className="ml-2 inline-flex items-center gap-1 text-[#8C2B1F] hover:text-[#8C2B1F] text-[11px]"><Trash2 size={12} />Remover</button>}
        </div>
      </div>
    </div>
  );
...
function TextSetting({ label, storageKey, placeholder }: { label: string; storageKey: string; placeholder: string }) {
  ...
  return (
    <label className="flex items-center gap-2 text-[12px]">
      ...
      <input value={val} placeholder={placeholder} onChange={(e) => save(e.target.value)} className="flex-1 border border-[#7FA9B1] px-2 py-1.5 bg-white" />
    </label>
  );
}
  ...
  return (
    <ClassicWindow title="Personalização / Aparência" icon={<Image size={14} className="text-gray-300" />}
      ...
      <div className="p-4 space-y-4 bg-[#F7FAFA] h-full overflow-auto">
        <div className="bg-[#F7FAFA] border border-[#CFE3E6] p-3">
          <div className="flex items-center gap-2 mb-3 text-[#5C8891] font-bold text-[12px]"><Type size={14} />Identidade</div>
          <div className="space-y-2">
            ...
            <label className="flex items-center gap-2 text-[12px]">
              ...
              <input type="color" value={bar} onChange={(e) => saveBar(e.target.value)} className="w-12 h-8 border border-[#7FA9B1] bg-white" />
              <span className="text-gray-500">{bar}</span>
              <button onClick={() => saveBar(DEFAULT_BAR_COLOR)} className="text-[#5C8891] hover:underline">repor</button>
            </label>
          </div>
        </div>
