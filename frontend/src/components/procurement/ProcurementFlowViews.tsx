    ...
    <ClassicWindow title="Requisições de Compra (com aprovação)" icon={<ClipboardList size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          <input placeholder="Requisitante" value={d.requester} onChange={(e) => setD({ ...d, requester: e.target.value })} className="border border-[#7FA9B1] p-1 w-32" />
          <select value={d.item} onChange={(e) => setD({ ...d, item: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
            <option value="">— artigo —</option>{items.map((i: any) => <option key={i.id} value={i.id}>[{i.code}] {i.name}</option>)}
          </select>
          <input placeholder="Qtd" type="number" value={d.quantity} onChange={(e) => setD({ ...d, quantity: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
          <select value={d.uom} onChange={(e) => setD({ ...d, uom: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
  ...
  return (
    <ClassicWindow title="RFQ · Cotações · Comparação → Ordem de Compra" icon={<Scale size={14} className="text-gray-300" />}
      ...
      <div className="flex h-full">
        <div className="w-56 border-r border-[#7FA9B1] overflow-auto">
          <div className="bg-[#F7FAFA] border-b border-[#7FA9B1] px-2 py-1.5 font-bold text-[11px] text-[#5C8891]">RFQs</div>
          {rfqs.map((r: any) => (
            <div key={r.id} onClick={() => setSel(r.id)} className={`px-3 py-2 text-[12px] cursor-pointer border-b border-[#EEF4F5] ${rfq?.id === r.id ? 'bg-[#F7FAFA]' : 'hover:bg-[#F7FAFA]'}`}>
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
            <span className="font-bold text-[#5C8891]">{rfq ? `Comparação · ${rfq.number}` : 'Selecione uma RFQ'}</span>
            {rfq && <>
              <select value={sup} onChange={(e) => setSup(e.target.value)} className="border border-[#7FA9B1] p-1 bg-white ml-auto">
          ...
          <div className="flex-1 overflow-auto p-2">
            {comp?.rows?.length ? (
              <table className="w-full text-[11px] border-collapse">
                <thead><tr className="bg-[#F7FAFA] text-[#5C8891]"><th className="text-left p-1 border border-[#CFE3E6]">Artigo</th><th className="p-1 border border-[#CFE3E6]">Qtd</th>
                  {(comp.quote_totals || []).map((q: any) => <th key={q.quote} className="p-1 border border-[#CFE3E6]">{q.supplier}</th>)}
                  <th className="p-1 border border-[#CFE3E6]">Melhor</th></tr></thead>
                <tbody>
                  {comp.rows.map((row: any) => (
                    <tr key={row.item}>
                      <td className="p-1 border border-[#EEF4F5]">{row.item_name}</td>
                      <td className="p-1 border border-[#EEF4F5] text-center">{Number(row.quantity).toFixed(0)}</td>
                  ...
                  <tr className="bg-[#F7FAFA] font-bold"><td className="p-1 border border-[#CFE3E6]" colSpan={2}>Total</td>
                    {(comp.quote_totals || []).map((q: any) => <td key={q.quote} className="p-1 border border-[#CFE3E6] text-right">{money(q.total)}</td>)}
                    <td className="p-1 border border-[#CFE3E6]" /></tr>
                </tbody>
              </table>
