...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="border border-[#7FA9B1] bg-white p-2">
    <h3 className="font-bold text-[#5C8891] border-b border-[#7FA9B1] mb-2 pb-1">{title}</h3>
    {children}
  </div>
);
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        {/* Barra de Tabs */}
        <div className="flex bg-[#EEF4F5] border-b border-[#7FA9B1] px-2 pt-1 flex-shrink-0">
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1 text-[11px] border border-b-0 mr-1 ${
                  active ? 'bg-white border-[#7FA9B1] font-bold text-[#5C8891]' : 'bg-[#F7FAFA] border-[#CFE3E6] text-gray-600'
                } ${disabled ? 'opacity-40 cursor-not-allowed' : 'hover:bg-[#FFFFFF]'}`}
              >
                {t.label}
              </button>
            );
        ...
        <div className="flex-1 overflow-y-auto bg-[#F7FAFA] p-4 text-[11px]">
          {tab === 'geral' && (
  ...
  return (
    <Section title="Contactos do Fornecedor">
      <div className="flex flex-wrap items-end gap-2 mb-3 bg-[#FFFFFF] p-2 border border-[#EEF4F5]">
        <select value={draft.role} onChange={(e) => setDraft({ ...draft, role: e.target.value })} className="border border-[#7FA9B1] p-1">
        ...
        <input placeholder="Cargo" value={draft.job_title || ''} onChange={(e) => setDraft({ ...draft, job_title: e.target.value })} className="border border-[#7FA9B1] p-1" />
        <input placeholder="Telefone" value={draft.phone || ''} onChange={(e) => setDraft({ ...draft, phone: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
        <input placeholder="Email" value={draft.email || ''} onChange={(e) => setDraft({ ...draft, email: e.target.value })} className="border border-[#7FA9B1] p-1" />
      ...
      <ClassicGrid
        ...
        columns={[
          ...
          { header: '', accessor: (r: any) => <button onClick={() => del.mutate(r.id)} className="text-[#8C2B1F] hover:text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '5%' },
        ]}
      />
    </Section>
  );
...
function CatalogTab({ supplierId }: { supplierId: number }) {
  ...
  return (
    <Section title="Catálogo de Produtos & Preços Acordados">
      <div className="flex flex-wrap items-end gap-2 mb-3 bg-[#FFFFFF] p-2 border border-[#EEF4F5]">
        <input placeholder="ID do Artigo" value={draft.item} onChange={(e) => setDraft({ ...draft, item: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
        <input placeholder="Cód. do Fornecedor" value={draft.supplier_item_code || ''} onChange={(e) => setDraft({ ...draft, supplier_item_code: e.target.value })} className="border border-[#7FA9B1] p-1" />
        <input placeholder="Preço Acordado" type="number" value={draft.agreed_price} onChange={(e) => setDraft({ ...draft, agreed_price: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
        <input placeholder="IVA %" type="number" value={draft.vat_percentage} onChange={(e) => setDraft({ ...draft, vat_percentage: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
        <ClassicButton icon={Plus} label="Adicionar" onClick={add} />
      </div>
      <ClassicGrid
        rowKey="id"
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      <Section title="Contratos">
        <div className="flex flex-wrap items-end gap-2 mb-3 bg-[#FFFFFF] p-2 border border-[#EEF4F5]">
          <input placeholder="Referência" value={c.reference} onChange={(e) => setC({ ...c, reference: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <label className="text-gray-600">Início <input type="date" value={c.start_date} onChange={(e) => setC({ ...c, start_date: e.target.value })} className="border border-[#7FA9B1] p-1" /></label>
          <label className="text-gray-600">Fim <input type="date" value={c.end_date} onChange={(e) => setC({ ...c, end_date: e.target.value })} className="border border-[#7FA9B1] p-1" /></label>
          <input placeholder="Incoterms" value={c.incoterms} onChange={(e) => setC({ ...c, incoterms: e.target.value })} className="border border-[#7FA9B1] p-1 w-24" />
        ...
        <ClassicGrid
          ...
          columns={[
            ...
            { header: '', accessor: (r: any) => <button onClick={() => delContract.mutate(r.id)} className="text-[#8C2B1F] hover:text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '10%' },
          ]}
        />
      </Section>
      ...
      <Section title="Documentos & Certificados (com alertas de validade)">
        <div className="flex flex-wrap items-end gap-2 mb-3 bg-[#FFFFFF] p-2 border border-[#EEF4F5]">
          <select value={d.document_type} onChange={(e) => setD({ ...d, document_type: e.target.value })} className="border border-[#7FA9B1] p-1">
          ...
          <input placeholder="Título" value={d.title} onChange={(e) => setD({ ...d, title: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <label className="text-gray-600">Validade <input type="date" value={d.expiration_date} onChange={(e) => setD({ ...d, expiration_date: e.target.value })} className="border border-[#7FA9B1] p-1" /></label>
...
function PerformanceTab({ supplier, onRecalc, recalculating }: { supplier?: Supplier; onRecalc: () => void; recalculating: boolean }) {
  ...
  const Metric = ({ label, value, suffix = '%' }: { label: string; value: any; suffix?: string }) => (
    <div className="bg-white border border-[#CFE3E6] p-3">
      ...
      <div className="text-xl font-bold text-[#5C8891]">{value ?? '—'}{value != null && suffix}</div>
    </div>
  );
