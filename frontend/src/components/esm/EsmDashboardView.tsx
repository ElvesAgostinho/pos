...
const Panel = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="bg-[#F7FAFA] border-2 border-white border-b-[#7FA9B1] border-r-[#7FA9B1] p-4">
    <h3 className="text-xs font-bold text-black mb-3 border-b border-[#7FA9B1] pb-1">{title}</h3>
    {children}
  </div>
);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                {(data?.top_suppliers?.length ?? 0) === 0 ? (
                  ...
                  <div className="space-y-2">
                    {data!.top_suppliers.map((s) => (
                      <div key={s.id} className="flex items-center justify-between bg-white border border-[#CFE3E6] px-2 py-1.5">
                        <div className="flex items-center gap-2 min-w-0">
                          <Award size={14} className="text-[#5C8891] flex-shrink-0" />
              ...
              <Panel title={`Certificados / Documentos a Expirar (30 dias)`}>
                {(data?.expired_documents ?? 0) > 0 && (
                  <div className="flex items-center gap-1 text-[11px] text-[#8C2B1F] font-bold mb-2">
                ...
                {(data?.expiring_documents?.length ?? 0) === 0 ? (
                  ...
                  <div className="space-y-1">
                    {data!.expiring_documents.map((d) => (
                      <div key={d.id} className="flex items-center justify-between bg-white border border-[#CFE3E6] px-2 py-1 text-[11px]">
                        <div className="min-w-0">
                ...
                {(data?.expiring_contracts?.length ?? 0) === 0 ? (
                  ...
                  <div className="space-y-1">
                    {data!.expiring_contracts.map((c) => (
                      <div key={c.id} className="flex items-center justify-between bg-white border border-[#CFE3E6] px-2 py-1 text-[11px]">
                        <div className="min-w-0">
