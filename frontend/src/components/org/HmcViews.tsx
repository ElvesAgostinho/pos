...
function HotelSelect({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="border border-[#7FA9B1] p-1 bg-white">
...
export function HmcDashboardView() {
  ...
  const Card = ({ label, value }: any) => (
    <div className="bg-white border border-[#7FA9B1] p-3"><div className="text-[10px] text-gray-500 uppercase">{label}</div><div className="text-2xl font-bold text-[#5C8891]">{value ?? '—'}</div></div>
  );
  return (
...
export function HmcFloorsView() {
  ...
  return (
    <ClassicWindow title="Pisos & Alas" icon={<Layers size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Pisos: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          <select value={bld} onChange={(e) => setBld(e.target.value)} className="border border-[#7FA9B1] p-1 bg-white">
            <option value="">Todos os edifícios</option>{buildings.map((b: any) => <option key={b.id} value={b.id}>{b.name}</option>)}
          </select>
          <span className="text-gray-400">|</span>
          <input type="number" placeholder="Nº piso" value={f.number} onChange={(e) => setF({ ...f, number: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
          <input placeholder="Nome/Ala" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <ClassicButton icon={Plus} label="Adicionar" onClick={add} />
        </div>
        <div className="flex-1 overflow-hidden">
          <ClassicGrid rowKey="id" data={rows} columns={[
  ...
  return (
    <ClassicWindow title={title} icon={icon} footer={<div className="text-gray-600">Centros: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          <HotelSelect value={f.hotel} onChange={(v) => setF({ ...f, hotel: v })} />
          <input placeholder="Código" value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} className="border border-[#7FA9B1] p-1 w-24" />
          <input placeholder="Nome" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
          {extraField === 'manager' && <input placeholder="Responsável" value={f.manager} onChange={(e) => setF({ ...f, manager: e.target.value })} className="border border-[#7FA9B1] p-1" />}
          <ClassicButton icon={Plus} label="Adicionar" onClick={add} />
        </div>
        <div className="flex-1 overflow-hidden">
          <ClassicGrid rowKey="id" data={rows} columns={[
    ...
    <ClassicWindow title="Recursos & Equipamentos" icon={<Wrench size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Recursos: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          ...
          <input placeholder="Código" value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
          <input placeholder="Nome" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <select value={f.resource_type} onChange={(e) => setF({ ...f, resource_type: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
            {Object.entries(RES_TYPES).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
          <input placeholder="Localização" value={f.location} onChange={(e) => setF({ ...f, location: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
          <select value={f.status} onChange={(e) => setF({ ...f, status: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
