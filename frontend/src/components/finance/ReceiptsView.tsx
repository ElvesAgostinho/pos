    ...
    <ClassicWindow title="Recebimentos (Financeiro)" icon={<ArrowDownCircle size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex flex-wrap items-end gap-2 bg-[#f0f0f0] border-b border-[#a0a0a0] px-3 py-2 text-[11px]">
          ...
          <input placeholder="Pagador" value={draft.party_name} onChange={(e) => setDraft({ ...draft, party_name: e.target.value })} className="border border-[#a0a0a0] p-1" />
        ...
        <div className="flex-1 overflow-hidden">
          <ClassicGrid
            ...
            columns={[
              ...
              { header: 'Pagador', accessor: 'party_name', width: '22%' },
              ...
              { header: '', accessor: (r: any) => r.status === 'DRAFT' ? <button title="Confirmar" onClick={() => confirm.mutate(r.id)} className="text-green-700 hover:text-green-900"><Check size={13} /></button> : null, width: '8%' },
            ]}
          />
        </div>
      </div>
