    ...
    <ClassicWindow title="Tesouraria — Contas (Financeiro)" icon={<Landmark size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          ...
          <select value={draft.account_type} onChange={(e) => setDraft({ ...draft, account_type: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
            <option value="CASH">Caixa</option><option value="BANK">Banco</option>
          </select>
          <input placeholder="Moeda" value={draft.currency} onChange={(e) => setDraft({ ...draft, currency: e.target.value.toUpperCase() })} className="border border-[#7FA9B1] p-1 w-20" />
          <input placeholder="Saldo inicial" type="number" value={draft.opening_balance} onChange={(e) => setDraft({ ...draft, opening_balance: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
