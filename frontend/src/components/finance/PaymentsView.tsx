    ...
    <ClassicWindow title="Pagamentos (Financeiro)" icon={<ArrowUpCircle size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          ...
          <input placeholder="Beneficiário" value={draft.party_name} onChange={(e) => setDraft({ ...draft, party_name: e.target.value })} className="border border-[#7FA9B1] p-1" />
        ...
        <div className="flex-1 overflow-hidden">
          <ClassicGrid
            ...
            columns={[
              ...
              { header: '', accessor: (r: any) => r.status === 'DRAFT' ? <button title="Confirmar" onClick={() => doConfirm(r.id)} className="text-[#0B4F5C] hover:text-[#062A31]"><Check size={13} /></button> : null, width: '8%' },
            ]}
          />
        </div>
      </div>
