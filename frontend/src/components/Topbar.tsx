...
const Brand = ({ size = 'text-xl' }: { size?: string }) => (
  <span className={`font-black tracking-tight ${size}`}><span className="text-[#062A31]">M</span><span className="text-white">L</span></span>
);
...
const MOD_COLOR: Record<string, string> = {
  ...
  commercial: '#062A31', srm: '#5C8891', procurement: '#5C8891', warehouse: '#5C8891', hospitality: '#5C8891',
  pms: '#5C8891', posmgmt: '#062A31', posfront: '#062A31', financial: '#5C8891', fiscal: '#B0392B',
  reporting: '#062A31', workflow: '#5C8891', documents: '#062A31', notifications: '#7FA9B1',
  integration: '#062A31', system: '#5C8891',
};
    ...
    <div className="flex items-center justify-between bg-[#062A31] text-white h-11 px-3 text-sm font-sans select-none relative z-50">
      ...
      <div ref={ref} className="relative">
        <button onClick={() => licensed.length > 1 && setOpen((o) => !o)}
          className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-[#041F24]">
          <Brand />
        ...
        {open && (
          <div className="absolute left-0 top-11 bg-[#062A31] border border-[#041F24] min-w-[280px] shadow-2xl py-1 z-50 max-h-[70vh] overflow-auto">
            <div className="px-3 py-1.5 flex items-center gap-2 border-b border-[#041F24]"><Brand size="text-base" /><span className="text-[10px] uppercase tracking-widest text-gray-500">Os seus módulos</span></div>
            {licensed.map((m) => (
