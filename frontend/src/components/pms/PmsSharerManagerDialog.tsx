...
export default function PmsSharerManagerDialog({ reservation: r, onClose }: { reservation: any; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9200] flex items-center justify-center bg-black/40">
      <div className="w-[820px] max-h-[70vh] bg-[#F7FAFA] border border-[#5C8891] shadow-xl rounded-[16px] overflow-hidden flex flex-col">
          ...
          <table className="w-full text-[12px] border-collapse">
            <thead style={{ background: '#F7FAFA' }}>
              <tr>{['Nº Quarto', 'Nome', 'Hóspede Principal', 'Check-In', 'Check-Out', 'Tipo de Reserva'].map((h) => (
                <th key={h} className="text-left px-3 py-1.5 border-b border-[#CFE3E6] font-semibold">{h}</th>))}
              </tr>
            </thead>
            <tbody>
              {rows.map((s: any) => (
