...
export default function WhCostingView() {
  ...
  return (
    <ClassicWindow title="Valorização de Stock (FIFO / FEFO)" icon={<Calculator size={14} className="text-gray-300" />}
      footer={<div className="text-gray-600">Valor total imobilizado: <span className="font-bold text-[#5C8891]">{AOA(d?.total_value)}</span></div>}>
      <div className="p-3 space-y-3">
        {isLoading || !d ? <div className="text-center text-gray-400 py-8 text-[12px]">A carregar…</div> : (
          <>
            <div className="bg-[#F7FAFA] border border-[#7FA9B1] px-3 py-2 text-[11px] text-gray-700">
              <b>Método de custeio:</b> {d.method}
            </div>
            <div className="bg-white border border-[#7FA9B1]">
              <div className="bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-1.5 text-[11px] font-bold text-gray-700">Valorização por armazém</div>
            ...
            <div className="bg-white border border-[#7FA9B1]">
              <div className="bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-1.5 text-[11px] font-bold text-gray-700">Top artigos por valor imobilizado</div>
