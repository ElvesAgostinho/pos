    ...
    <ClassicWindow title="Lotes & Validades (FEFO)" icon={<CalendarX size={14} className="text-gray-300" />}
      ...
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          <select value={f.warehouse} onChange={(e) => setF({ ...f, warehouse: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
            <option value="">Armazém…</option>
            {warehouses.map((w: any) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>
          <input placeholder="Pesquisar artigo" value={itemSearch} onChange={(e) => setItemSearch(e.target.value)} className="border border-[#7FA9B1] p-1 w-32" />
          <select value={f.item} onChange={(e) => setF({ ...f, item: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white max-w-[160px]">
            <option value="">Artigo…</option>
            {items.map((i: any) => <option key={i.id} value={i.id}>{i.name}</option>)}
          </select>
          <input placeholder="Nº lote" value={f.lot_number} onChange={(e) => setF({ ...f, lot_number: e.target.value })} className="border border-[#7FA9B1] p-1 w-24" />
          <input type="number" placeholder="Qtd" value={f.quantity} onChange={(e) => setF({ ...f, quantity: e.target.value })} className="border border-[#7FA9B1] p-1 w-16" />
          <input type="date" value={f.expiry_date} onChange={(e) => setF({ ...f, expiry_date: e.target.value })} className="border border-[#7FA9B1] p-1" />
