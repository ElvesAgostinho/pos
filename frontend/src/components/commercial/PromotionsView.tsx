    ...
    <ClassicWindow title="Promoções & Happy Hour (Commercial → alimenta o POS)" icon={<Tag size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          ...
          <select value={d.scope} onChange={(e) => setD({ ...d, scope: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
            <option value="ALL">Todos os artigos</option><option value="CATEGORY">Categoria</option><option value="ITEM">Artigo</option>
          </select>
          {d.scope === 'CATEGORY' && (
            <select value={d.category} onChange={(e) => setD({ ...d, category: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
          ...
          <select value={d.discount_type} onChange={(e) => setD({ ...d, discount_type: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
            <option value="PERCENT">%</option><option value="FIXED">Valor fixo</option>
          </select>
          <input placeholder="Valor" type="number" value={d.value} onChange={(e) => setD({ ...d, value: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
          <label className="text-gray-600">HH<input type="time" value={d.happy_start} onChange={(e) => setD({ ...d, happy_start: e.target.value })} className="border border-[#7FA9B1] p-1 ml-1" /></label>
          <label className="text-gray-600">–<input type="time" value={d.happy_end} onChange={(e) => setD({ ...d, happy_end: e.target.value })} className="border border-[#7FA9B1] p-1 ml-1" /></label>
