  ...
  return (
    <ClassicWindow title="Tabelas de Preço (mesmo artigo, preço por área)" icon={<Tags size={14} className="text-gray-300" />}
      ...
      <div className="flex h-full">
        {/* Tabelas */}
        <div className="w-64 border-r border-[#7FA9B1] flex flex-col">
          <div className="bg-[#F7FAFA] border-b border-[#7FA9B1] p-2 space-y-1 text-[11px]">
            <input placeholder="Código" value={draft.code} onChange={(e) => setDraft({ ...draft, code: e.target.value.toUpperCase() })} className="border border-[#7FA9B1] p-1 w-full" />
            <input placeholder="Nome da tabela" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} className="border border-[#7FA9B1] p-1 w-full" />
            <ClassicButton icon={Plus} label="Nova Tabela" onClick={addList} />
          </div>
          <div className="flex-1 overflow-auto">
            {lists.map((l: any) => (
              <div key={l.id} onClick={() => setSel(l.id)} className={`flex items-center justify-between px-3 py-2 text-[12px] cursor-pointer border-b border-[#EEF4F5] ${active?.id === l.id ? 'bg-[#F7FAFA]' : 'hover:bg-[#F7FAFA]'}`}>
                <span><b>{l.name}</b><br /><span className="text-gray-500 text-[10px]">{l.code} · {l.item_count ?? (l.items?.length || 0)} artigos</span></span>
                <button onClick={(e) => { e.stopPropagation(); del.mutate(l.id); }} className="text-[#8C2B1F] hover:text-[#8C2B1F]"><Trash2 size={12} /></button>
              </div>
            ))}
        ...
        <div className="flex-1 flex flex-col">
          <div className="bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px] flex flex-wrap items-end gap-2">
            <span className="font-bold text-[#5C8891]">{active ? `Preços · ${active.name}` : 'Selecione uma tabela'}</span>
            {active && <>
              <select value={line.item} onChange={(e) => setLine({ ...line, item: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white ml-auto">
                <option value="">— artigo —</option>{items.map((i: any) => <option key={i.id} value={i.id}>[{i.code}] {i.name}</option>)}
              </select>
              <input placeholder="Preço" type="number" value={line.price} onChange={(e) => setLine({ ...line, price: e.target.value })} className="border border-[#7FA9B1] p-1 w-24" />
          ...
          <div className="flex-1 overflow-hidden">
            <ClassicGrid rowKey="id" data={active?.items || []} columns={[
              ...
              { header: '', accessor: (r: any) => <button onClick={() => active && removeItem.mutate({ id: active.id as number, item: r.item })} className="text-[#8C2B1F] hover:text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '10%' },
            ]} />
          </div>
        </div>
      </div>
