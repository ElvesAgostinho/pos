...
export default function ComponentsPicker({ onClose, onPick }: {
  ...
  const inp = 'border border-[#7FA9B1] px-2 py-1 text-[12px] w-full';
      ...
      <div className="bg-white w-[1100px] max-w-[95vw] h-[720px] max-h-[92vh] flex flex-col shadow-2xl rounded-[16px] overflow-hidden"
        ...
        <div className="flex-1 overflow-auto">
          <table className="w-full text-[12px] border-collapse">
            <thead className="sticky top-0 bg-[#F7FAFA]">
              <tr>
                <th className="w-[28px] border border-[#EEF4F5]"></th>
            ...
            <tbody>
              {linhas.map((it: any) => (
                <tr key={it.id} onClick={() => setSel(it)} onDoubleClick={() => onPick(it)}
                  className={`cursor-pointer ${sel?.id === it.id ? 'bg-[#F7FAFA]' : 'hover:bg-[#F7FAFA]'}`}>
                  <td className="text-center border border-[#F7FAFA]">
                    <input type="radio" checked={sel?.id === it.id} onChange={() => setSel(it)} />
                  </td>
                  <td className="px-2 py-1 border border-[#F7FAFA]">{it.code}</td>
                  <td className="px-2 py-1 border border-[#F7FAFA]">{it.name}</td>
                  <td className="px-2 py-1 border border-[#F7FAFA]">{it.group_name || '—'}</td>
                  <td className="px-2 py-1 border border-[#F7FAFA]">{it.family_name || '—'}</td>
                  <td className="px-2 py-1 border border-[#F7FAFA]">{it.subfamily_name || '—'}</td>
                  <td className="px-2 py-1 border border-[#F7FAFA] text-right">{money(it.sale_price)}</td>
                  <td className="px-2 py-1 border border-[#F7FAFA] text-center">{it.is_active ? <Glyph icon="✔" size={13} /> : '—'}</td>
                </tr>
              ))}
              {!isFetching && linhas.length === 0 && (
                <tr><td colSpan={8} className="text-center text-[#7FA9B1] py-8">Sem artigos com estes filtros.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="flex items-center justify-between px-3 py-2 border-t border-[#EEF4F5] bg-[#F7FAFA] text-[12px]">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPage(1)} disabled={page <= 1} className="px-2 py-1 border border-[#EEF4F5] disabled:opacity-30 inline-flex"><Glyph icon="⏮" size={13} /></button>
            ...
            <button onClick={() => setPage(totalPaginas)} disabled={page >= totalPaginas} className="px-2 py-1 border border-[#EEF4F5] disabled:opacity-30 inline-flex"><Glyph icon="⏭" size={13} /></button>
          </div>
        </div>
