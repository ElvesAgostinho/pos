  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#F7FAFA] border-b border-[#EEF4F5]">
        <span className="text-[13px] font-bold text-[#06333C]">{isNew ? 'Nova sub-família' : `A editar ${d.name}`}</span>
      ...
      <div className="flex-1 overflow-auto p-4">
        ...
        <div className="space-y-2 pt-1.5">
          <label className="flex items-center gap-3 text-[13px]">
            <span className="w-[200px] text-[#06333C]">Código:<span className="text-[#B0392B]">*</span></span>
            <input value={d.code || ''} onChange={(e) => set('code', e.target.value)} className={`${inputCls} w-[290px] flex-none`} style={inputStyle} />
          </label>
          <label className="flex items-center gap-3 text-[13px]">
            <span className="w-[200px] text-[#06333C]">Descrição:<span className="text-[#B0392B]">*</span></span>
            <input value={d.name || ''} onChange={(e) => set('name', e.target.value)} className={`${inputCls} w-[640px] flex-none`} style={inputStyle} />
          </label>
          <label className="flex items-center gap-3 text-[13px]">
            <span className="w-[200px] text-[#06333C]">Família:<span className="text-[#B0392B]">*</span></span>
          ...
          <label className="flex items-center gap-3 text-[13px]">
            <span className="w-[200px] text-[#06333C]">Ordem impressão no pedido:</span>
          ...
          <label className="flex items-center gap-3 text-[13px]">
            <span className="w-[200px] text-[#06333C]">Conta de Contabilidade:</span>
            <input value={d.accounting_account || ''} onChange={(e) => set('accounting_account', e.target.value)}
              className={`${inputCls} w-[210px] flex-none`} style={inputStyle} />
          </label>
          <label className="flex items-center gap-3 text-[13px]">
            <span className="w-[200px] text-[#06333C]">IVA por omissão (%):</span>
            ...
            <div className="flex">
              {([['pms', 'Interface com PMS (Mapeamento de sub-famílias)'], ['wh', 'Mapeamentos Setor/Armazéns']] as const).map(([k, label]) => (
                <button key={k} onClick={() => setTab(k)}
                  className={`px-4 py-2 text-[13px] font-semibold border border-b-0 ${tab === k ? 'bg-white text-[#062A31] border-[#CFE3E6]' : 'bg-[#F7FAFA] text-[#0B4F5C] border-[#EEF4F5]'}`}>
                  {label}
                </button>
              ))}
            </div>

            <div className="p-3" style={{ border: '4px groove #CFE3E6' }}>
              ...
              <div className="flex items-center gap-3 mb-3 text-[13px]">
                ...
                <button onClick={applyToAll} className="flex items-center gap-1.5 text-[13px] text-[#0B4F5C] font-semibold">
