...
export default function BarcodePrint() {
  ...
  const print = () => {
    ...
    const html = `<html><head><title>Etiquetas</title><style>
      ...
      .lbl{display:inline-block;width:180px;height:90px;border:1px solid #EEF4F5;margin:4px;padding:6px;text-align:center;vertical-align:top}
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-white">
      ...
      {/* Barra de seleção */}
      <div className="flex items-center gap-5 px-4 py-2 bg-[#F7FAFA] border-b border-[#EEF4F5] text-[13px]">
        ...
        <label className="flex items-center gap-2">
          ...
          <input type="number" min={1} value={qty} onChange={(e) => setQty(Number(e.target.value) || 1)}
            className="border border-[#7FA9B1] px-2 py-1 text-[12px] w-[80px] bg-white" style={inputStyle} />
        </label>
        <label className="flex items-center gap-2">
          Modelo de Impressão:
          <select value={model} onChange={(e) => setModel(e.target.value)}
            className="border border-[#7FA9B1] px-2 py-1 text-[12px] w-[220px] bg-white" style={inputStyle}>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-[12px] border-collapse">
          ...
          <tbody>
            {rows.map((r: any) => (
              <tr key={r.id} className="border-b border-[#F7FAFA] hover:bg-[#FFFFFF]">
                <td className="text-center border border-[#F7FAFA]">
                  <input type="checkbox" checked={!!sel[r.id]} onChange={() => toggle(r.id)} className="w-4 h-4" />
                </td>
                <td className="px-2 py-1 border border-[#F7FAFA] font-mono">{r.code}</td>
                <td className="px-2 py-1 border border-[#F7FAFA]">{r.code}</td>
                <td className="px-2 py-1 border border-[#F7FAFA]">{r.name}</td>
                <td className="px-2 py-1 border border-[#F7FAFA] text-right">{money(priceOf(r))}</td>
                <td className="px-2 py-1 border border-[#F7FAFA]">{r.units_label?.split('/').pop()?.trim()}</td>
                <td className="px-1 py-0.5 border border-[#F7FAFA] w-[90px]">
                  <input type="number" min={1} value={sel[r.id] || ''} disabled={!sel[r.id]}
                    onChange={(e) => setSel((s) => ({ ...s, [r.id]: Number(e.target.value) || 1 }))}
                    className="w-full border border-[#EEF4F5] px-1 py-0.5 text-[12px] text-right disabled:bg-[#F7FAFA]" />
                </td>
              </tr>
            ))}
            {rows.length === 0 && (
              <tr><td colSpan={7} className="text-center text-[#7FA9B1] py-10">
