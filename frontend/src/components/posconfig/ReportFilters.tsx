  ...
  return (
    <div className="border border-[#CFE3E6] bg-white">
      <button onClick={() => setAberto(!aberto)}
        ...
        <span className="ml-auto text-[13px] text-[#5C8891]">{aberto ? '−' : '+'}</span>
      </button>
        ...
        <div className="p-4 space-y-4">
                ...
                <div key={x.key} className="flex items-center gap-2">
                  ...
                  {x.type === 'warehouse' ? (
                    ...
                    <input
                      ...
                      onChange={(e) => setValues?.({ ...values, [x.key]: e.target.value })}
                      className={`${inp} w-[160px] disabled:bg-[#F7FAFA] disabled:text-[#7FA9B1]`}
                      style={inputStyle} />
                  )}
                </div>
              ))}
            ...
            {presets.map((p: any) => (
              <button key={p.value}
                ...
                className={`px-3 py-1 text-[12px] border ${preset === p.value
                  ? 'bg-[#5C8891] text-white border-[#5C8891] font-semibold'
                  : 'bg-white border-[#CFE3E6] hover:bg-[#FFFFFF]'}`}>
                {p.label}
              </button>
            ))}
            {preset && <span className="text-[11px] text-[#5C8891]">(ignora as datas acima)</span>}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => set('weekdays', dias.length === 7 ? [] : [0, 1, 2, 3, 4, 5, 6])}
              className="ml-2 text-[11px] text-[#5C8891] hover:underline">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-[#5C8891]">na coluna de dinheiro do relatório</span>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-[#5C8891]">procura em todas as colunas</span>
          </div>
          ...
          <div className="flex items-center gap-3 pt-2 border-t border-[#F7FAFA]">
            <button onClick={() => { setAdv({}); setPreset(''); }}
              className="px-3 py-1 text-[12px] border border-[#B0392B] text-[#B0392B]">
              Limpar filtro
            </button>
            <span className="text-[11px] text-[#5C8891]">
