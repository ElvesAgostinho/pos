    ...
    >
      <div className="flex flex-col h-full">
        <div className="flex items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          <input placeholder="Marca" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <input placeholder="Fabricante (opcional)" value={draft.manufacturer} onChange={(e) => setDraft({ ...draft, manufacturer: e.target.value })} className="border border-[#7FA9B1] p-1" />
