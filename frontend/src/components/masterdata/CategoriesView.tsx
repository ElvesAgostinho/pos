    ...
    >
      <div className="flex flex-col h-full">
        <div className="flex items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          <input placeholder="Nome da categoria" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <select value={draft.parent} onChange={(e) => setDraft({ ...draft, parent: e.target.value === '' ? '' : Number(e.target.value) })} className="border border-[#7FA9B1] p-1 bg-white">
