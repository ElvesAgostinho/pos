    ...
    <ClassicWindow title="Artigo Enterprise — Variantes, Unidades & Custo" icon={<Boxes size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          ...
          <select value={itemId || ''} onChange={(e) => setItemId(e.target.value ? Number(e.target.value) : undefined)} className="border border-[#7FA9B1] p-1 bg-white min-w-[240px]">
        ...
        {!itemId ? <div className="flex-1 flex items-center justify-center text-gray-400 text-sm">Selecione um artigo para configurar variantes, unidades e custo.</div> : (
          <div className="flex-1 grid grid-cols-2 gap-3 p-3 overflow-auto">
            {/* Variantes / Doses */}
            <div className="border border-[#CFE3E6] flex flex-col min-h-0">
              <div className="bg-[#F7FAFA] px-2 py-1 font-bold text-[#5C8891] text-[11px]">Variantes / Doses (½ dose, garrafa, copo…)</div>
              <div className="flex flex-wrap items-end gap-1 p-2 text-[11px] border-b border-[#EEF4F5]">
                <input placeholder="Código" value={vd.code} onChange={(e) => setVd({ ...vd, code: e.target.value.toUpperCase() })} className="border border-[#7FA9B1] p-1 w-20" />
                <input placeholder="Nome" value={vd.name} onChange={(e) => setVd({ ...vd, name: e.target.value })} className="border border-[#7FA9B1] p-1 w-24" />
                <input placeholder="Preço" type="number" value={vd.sale_price} onChange={(e) => setVd({ ...vd, sale_price: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
                <input placeholder="Fator" type="number" value={vd.qty_factor} onChange={(e) => setVd({ ...vd, qty_factor: e.target.value })} className="border border-[#7FA9B1] p-1 w-16" title="Unidades-base consumidas (½=0.5)" />
              ...
              <div className="flex-1 overflow-auto">
                <ClassicGrid rowKey="id" data={variants} columns={[
                  ...
                  { header: '', accessor: (r: any) => <button onClick={() => dv.mutate(r.id)} className="text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '12%' },
                ]} />
              </div>
            </div>
            ...
            <div className="border border-[#CFE3E6] flex flex-col min-h-0">
              <div className="bg-[#F7FAFA] px-2 py-1 font-bold text-[#5C8891] text-[11px]">Unidades com conversão (comprar em Caixa, vender em Unidade)</div>
              <div className="flex flex-wrap items-end gap-1 p-2 text-[11px] border-b border-[#EEF4F5]">
                <select value={ud.uom} onChange={(e) => setUd({ ...ud, uom: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
                  <option value="">— unidade —</option>{uoms.map((u: any) => <option key={u.id} value={u.id}>{u.code} · {u.name}</option>)}
                </select>
                <input placeholder="Fator" type="number" value={ud.factor} onChange={(e) => setUd({ ...ud, factor: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" title="Unidades-base por 1 desta (Caixa 24 → 24)" />
                <select value={ud.role} onChange={(e) => setUd({ ...ud, role: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">
              ...
              <div className="flex-1 overflow-auto">
                <ClassicGrid rowKey="id" data={itemUoms} columns={[
                  ...
                  { header: '', accessor: (r: any) => <button onClick={() => du.mutate(r.id)} className="text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '12%' },
                ]} />
              </div>
            </div>

            {/* Ficha técnica → custo */}
            <div className="col-span-2 border border-[#CFE3E6] p-3 text-[12px] bg-white">
              <div className="font-bold text-[#5C8891] mb-1">Ficha Técnica → Custo real</div>
              {recipe ? (
