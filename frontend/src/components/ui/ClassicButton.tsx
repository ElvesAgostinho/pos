...
export default function ClassicButton({ icon: Icon, iconColor = 'text-gray-700', label, children, className = '', ...props }: ClassicButtonProps) {
  ...
  return (
    <button
      className={`flex items-center gap-1.5 px-3 py-1.5 border text-[11px] font-semibold text-[#2a3543] focus:outline-none focus:ring-1 focus:ring-[#B08D3C] transition-none ${props.disabled ? 'opacity-45 cursor-not-allowed' : 'active:translate-y-px'} ${className}`}
      style={{
