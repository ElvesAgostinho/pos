...
function Card({ label, value, tone, sub }: any) {
  return (
    <div className="bg-white border border-[#7FA9B1] p-3">
      ...
      <div className={`text-2xl font-bold ${tone || 'text-[#5C8891]'}`}>{value ?? '—'}</div>
    ...
    <ClassicWindow title="Workflow — Fluxos" icon={<GitBranch size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Fluxos: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          <input placeholder="Nome do fluxo" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <input placeholder="Evento gatilho" value={f.trigger_event} onChange={(e) => setF({ ...f, trigger_event: e.target.value })} className="border border-[#7FA9B1] p-1 w-40" />
          <input placeholder="Descrição" value={f.description} onChange={(e) => setF({ ...f, description: e.target.value })} className="border border-[#7FA9B1] p-1" />
    ...
    <ClassicWindow title="Workflow — Tarefas" icon={<ListTodo size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Tarefas: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          ...
          <input placeholder="Responsável" value={f.assignee} onChange={(e) => setF({ ...f, assignee: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
          <select value={f.priority} onChange={(e) => setF({ ...f, priority: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white">{Object.entries(PR).map(([k, v]) => <option key={k} value={k}>{v}</option>)}</select>
          <select value={f.flow} onChange={(e) => setF({ ...f, flow: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white"><option value="">Fluxo…</option>{flows.map((fl: any) => <option key={fl.id} value={fl.id}>{fl.name}</option>)}</select>
          <input type="date" value={f.due_date} onChange={(e) => setF({ ...f, due_date: e.target.value })} className="border border-[#7FA9B1] p-1" />
          <ClassicButton icon={Plus} label="Criar" onClick={add} />
        </div>
        <div className="flex-1 overflow-hidden">
          <ClassicGrid rowKey="id" data={rows} columns={[
      ...
      <div className="flex h-full">
        <div className="w-1/2 border-r border-[#7FA9B1] flex flex-col">
          <div className="flex flex-wrap items-end gap-2 p-2 bg-[#F7FAFA] border-b border-[#7FA9B1] text-[11px]">
            <select value={hdr.supplier} onChange={(e) => setHdr({ ...hdr, supplier: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white max-w-[130px]"><option value="">Fornecedor…</option>{suppliers.map((s: any) => <option key={s.id} value={s.id}>{s.commercial_name}</option>)}</select>
            <select value={hdr.warehouse} onChange={(e) => setHdr({ ...hdr, warehouse: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white"><option value="">Armazém…</option>{warehouses.map((w: any) => <option key={w.id} value={w.id}>{w.name}</option>)}</select>
  ...
  return (
    <ClassicWindow title="Comercial — Fidelização (Loyalty)" icon={<Award size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Programas: {rows.length}</div>}>
      <div className="flex h-full">
        <div className="w-1/2 border-r border-[#7FA9B1] flex flex-col">
          <div className="flex flex-wrap items-end gap-2 p-2 bg-[#F7FAFA] border-b border-[#7FA9B1] text-[11px]">
            <input placeholder="Programa" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} className="border border-[#7FA9B1] p-1" />
            <input type="number" placeholder="Pts/Kz" value={f.points_per_currency} onChange={(e) => setF({ ...f, points_per_currency: e.target.value })} className="border border-[#7FA9B1] p-1 w-16" title="Pontos por Kz gasto" />
            <input type="number" placeholder="Kz/pt" value={f.redeem_value} onChange={(e) => setF({ ...f, redeem_value: e.target.value })} className="border border-[#7FA9B1] p-1 w-16" title="Valor de resgate por ponto" />
          ...
          <div className="flex-1 overflow-hidden">
            <ClassicGrid rowKey="id" data={rows} selectedRowId={selId ?? undefined} onRowClick={(r: any) => setSelId(r.id)} columns={[
              ...
              { header: '', accessor: (r: any) => <button onClick={(e) => { e.stopPropagation(); remove.mutate(r.id); }} className="text-[#8C2B1F] hover:text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '10%' },
            ]} />
          </div>
        </div>
        <div className="w-1/2 flex flex-col">
          {sel ? <>
            <div className="flex flex-wrap items-end gap-2 p-2 bg-[#F7FAFA] border-b border-[#7FA9B1] text-[11px]">
              <input placeholder="Escalão (Ouro…)" value={t.name} onChange={(e) => setT({ ...t, name: e.target.value })} className="border border-[#7FA9B1] p-1 w-24" />
              <input type="number" placeholder="Min. pts" value={t.min_points} onChange={(e) => setT({ ...t, min_points: e.target.value })} className="border border-[#7FA9B1] p-1 w-20" />
              <input type="number" placeholder="Desc.%" value={t.discount_percent} onChange={(e) => setT({ ...t, discount_percent: e.target.value })} className="border border-[#7FA9B1] p-1 w-16" />
              <input placeholder="Benefícios" value={t.benefits} onChange={(e) => setT({ ...t, benefits: e.target.value })} className="border border-[#7FA9B1] p-1" />
            ...
            <div className="flex-1 overflow-hidden">
              <ClassicGrid rowKey="id" data={tiers.rows} columns={[
                ...
                { header: '', accessor: (r: any) => <button onClick={() => tiers.remove.mutate(r.id)} className="text-[#8C2B1F] hover:text-[#8C2B1F]"><Trash2 size={12} /></button>, width: '8%' },
              ]} />
            </div>
...
export function FinReconciliationView() {
  ...
  return (
    <ClassicWindow title="Finanças — Reconciliação Bancária" icon={<Banknote size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Reconciliações: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          <select value={f.account} onChange={(e) => setF({ ...f, account: e.target.value })} className="border border-[#7FA9B1] p-1 bg-white"><option value="">Conta…</option>{accounts.map((a: any) => <option key={a.id} value={a.id}>{a.name} ({a.code})</option>)}</select>
    ...
    <ClassicWindow title="Administração — Grupos Empresariais" icon={<Building size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Grupos: {rows.length}</div>}>
      <div className="p-2 space-y-2 h-full flex flex-col">
        <div className="flex items-end gap-2 bg-[#F7FAFA] border border-[#7FA9B1] p-2 text-[11px]">
          <input placeholder="Nome do grupo" value={name} onChange={(e) => setName(e.target.value)} className="border border-[#7FA9B1] p-1" />
  ...
  return (
    <ClassicWindow title="Administração — Versões" icon={<ShieldCheck size={14} className="text-gray-300" />} footer={<div className="text-gray-600">Informação de versão e ambiente</div>}>
      <div className="p-3">{!d ? <div className="text-center text-gray-400 py-8 text-[12px]">A carregar…</div> : (
        <div className="bg-white border border-[#7FA9B1] p-3 max-w-lg">
  ...
  return (
    <ClassicWindow title={title} icon={icon} footer={<div className="text-gray-600">Política de segurança global</div>}>
      <div className="p-3 max-w-md space-y-2">
        {fields.map((fl) => (
          <div key={fl.key} className="flex items-center justify-between text-[12px] border-b border-[#F7FAFA] py-1.5">
            ...
            {fl.type === 'bool'
              ...
              : <input type={fl.type || 'number'} value={p[fl.key] ?? ''} onChange={(e) => setForm({ ...p, [fl.key]: fl.type === 'text' ? e.target.value : Number(e.target.value) })} className="border border-[#7FA9B1] p-1 w-32" />}
          </div>
        ))}
