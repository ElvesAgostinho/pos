    ...
    <ClassicWindow title="Gift Cards / Vouchers (POS · Motor 6)" icon={<Gift size={14} className="text-gray-300" />}
      ...
      <div className="flex flex-col h-full">
        <div className="flex flex-wrap items-end gap-2 bg-[#F7FAFA] border-b border-[#7FA9B1] px-3 py-2 text-[11px]">
          <input placeholder="Código" value={draft.code} onChange={(e) => setDraft({ ...draft, code: e.target.value.toUpperCase() })} className="border border-[#7FA9B1] p-1 w-40" />
          <input placeholder="Saldo inicial" type="number" value={draft.initial_balance} onChange={(e) => setDraft({ ...draft, initial_balance: e.target.value })} className="border border-[#7FA9B1] p-1 w-28" />
