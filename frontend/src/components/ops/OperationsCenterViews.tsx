...
function Panel({ title, icon: Icon, color, children }: any) {
  return (
    <div className="bg-white border border-[#CFE3E6] shadow-[inset_1px_1px_0_#FFFFFF]">
...
function Stat({ label, value, tone = '#5C8891' }: any) {
    ...
    <ClassicWindow title="Centro de Operações — Painel do Proprietário" icon={<Radar size={14} className="text-gray-300" />}
      ...
      <div className="p-4 bg-[#F7FAFA] h-full overflow-auto">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <Panel title="COZINHA (KDS)" icon={ChefHat} color="#5C8891">
          ...
          <Panel title="HOTEL (PMS)" icon={BedDouble} color="#5C8891">
            <div className="grid grid-cols-2 gap-2">
...
export function LiveTablesMonitor() {
  ...
  return (
    <ClassicWindow title="Monitor de Mesas — Tempo Real" icon={<Table2 size={14} className="text-gray-300" />}
      ...
      <div className="p-4 bg-[#062A31] h-full overflow-auto">
          ...
          {tables.map((t: any) => {
            ...
            return (
              <div key={t.id} className="rounded-lg p-2 text-white text-center shadow" style={{ background: COLOR[t.status] || '#06333C' }}>
                ...
                {tk ? (
                  <>
                    <div className="text-[15px] font-bold text-[#CFE3E6]">{money(tk.grand_total)}</div>
